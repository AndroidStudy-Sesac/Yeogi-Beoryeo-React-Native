...
function navigationState(offset = 612, detail = true): NavigationState {
  ...
  return { key: 'root', type: 'stack', stale: false, index: routes.length - 1,
...
it('같은 Android 작업이 복원되면 편집 입력, 제출 검색어, 스크롤과 상세 ID를 복원합니다', async () => {
  await createSearchSessionWriter('android-task-1')(navigationState());
  expect(await readSearchSession('android-task-1')).toEqual({ index: 1, routes: [
...
it('상세에서 돌아온 마지막 스크롤을 순서대로 저장하고 상세 화면을 다시 열지 않습니다', async () => {
  ...
  const first = write(navigationState());
  const last = write(navigationState(900, false));
  await Promise.all([first, last]);
  const restored = await readSearchSession('task');
...
it('상세를 읽던 위치도 검색 결과의 스크롤과 별도로 복원합니다', async () => {
  ...
  await createSearchSessionWriter('task')({ ...state, routes: state.routes.map(route =>
    route.name === 'ItemGuideDetail' ? { ...route, params: { guideId: 'item_001', scrollOffset: 500 } } : route) });
  expect((await readSearchSession('task'))?.routes[1].params).toEqual({
...
it('저장소 오류 뒤에도 읽기가 끝나며 다음 저장을 수행할 수 있습니다', async () => {
  ...
  await write(navigationState());
  await write(navigationState(100, false));
  expect((await readSearchSession('task'))?.routes[0].params).toEqual({
